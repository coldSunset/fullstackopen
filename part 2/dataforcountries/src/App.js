  ...
  const FilterQueryTest = () => {
    ...
    if(!(newCountry.trim()==""))
    {
      ...
      if(filterList.length > 10){
        ...
        return(<div><ul>
            {filterList.map(country=><ShowCountry key={country.name} country={country}/>
            )}
          </ul></div>)
      }
      else if(filterList.length == 1){
        return(<ShowCountry country = {filterList[0]}/>)
      }
      }
